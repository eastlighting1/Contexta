...
| 무증상 실패 | 대시보드 임계값 알림 | `DegradedRecord` + `diagnose_run` — 종료 코드 0이어도 감지 |
...
- **부가적, 강제적이지 않음.** 이미 만들어진 실행(run)과 스테이지에 레코드를 붙이는 방식입니다. Contexta의 추상화에 맞게 학습 코드를 재구성할 필요가 없습니다.
...
2. 학습 시점의 평가 메트릭은 무엇이었나요 (요약이 아닌 원본 수치)?
