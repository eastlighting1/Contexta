...
`Contexta`는 로컬 우선(local-first) ML 관측성(observability)과 하네스 엔지니어링(Harness Engineering) 워크플로를 위한 표준 제품 인터페이스입니다. 모델의 품질 저하를 조사하는 인간 운영자이든, 혹은 생성자-평가자 루프(Generator-Evaluator loop) 속에서 구조화된 증거를 읽고 판단하는 자율 AI 코딩 에이전트이든, Contexta는 기계 가독성(machine-readable)이 뛰어난 안정적인 진실의 원천(Source of truth)을 제공합니다. 이 문서 세트는 신규 사용자가 빠르게 시작하고, 고급 사용자가 시스템의 공식적인 인터페이스 지원 범위를 명확히 파악할 수 있으며, 운영자(인간과 AI 모두 포함)가 공식 가이드와 내부 구현 노트를 혼동하지 않고 복구 작업을 수행할 수 있도록 구성되었습니다.
...
### 개념적 개요 (Conceptual Overview)

시작하기 전에 "어떻게(how)" 보다 "왜(why)" 그런지 먼저 이해하고 싶다면 다음 문서로 시작하세요:

1. [ML 관측성이란 무엇인가?](./what-is-ml-observability.md)

이 문서는 일반적인 소프트웨어 관측성과 ML 관측성의 철학적 차이를 설명하며, `Contexta`가 메트릭 및 트레이스뿐만 아니라 아티팩트, 기능 저하(Degraded) 상태, 리니지를 필요로 하는 이유를 설명합니다.
...
레퍼런스 계층은 다음과 같은 안정적인 공식 규약(Public Contracts)을 담고 있습니다:
...
공식 문서 세트는 점진적으로 채워지고 있습니다. 현재 단계에서:

- README는 주요 퀵스타트 문서입니다.
- 이 허브는 공식 문서의 구조를 정의합니다.
