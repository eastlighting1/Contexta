...
`Contexta`는 로컬 우선(local-first) ML 관측성(observability) 워크플로를 위한 표준 제품 인터페이스입니다. 이 문서 세트는 신규 사용자가 빠르게 시작하고, 고급 사용자가 시스템의 공식적인 인터페이스 지원 범위를 명확히 파악할 수 있으며, 운영자가 공식 가이드와 내부 구현 노트를 혼동하지 않고 복구 작업을 수행할 수 있도록 구성되었습니다.
